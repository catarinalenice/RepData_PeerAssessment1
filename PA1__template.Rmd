...
Now, getting the data and storing in a variable, excluding the rows with missing data.
...
Third: Calculate and report the mean and median of the total number of steps taken per day
...
Moving on for the second question: What is the average daily activity pattern?

First: Make a time series plot (type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all days (y-axis)
...
For the rest of the assignment the data with missing values will be needed, so here we'll import the data again:
...
To answer the question, here is a plot comparing the values when calculated with an without the missing values:
```{r, echo = FALSE}

p1 <- ggplot(mean_steps_perday, aes(x = steps )) +
  geom_histogram() +
  labs(title = "Mean without NAs", x = "Mean per day", y = "")

p2 <- ggplot(median_steps_perday, aes(x = steps )) +
  geom_histogram() +
  labs(title = "Median without NAs", x = "Median per day", y = "")

p3 <- ggplot(mean_filled_NA, aes(x = steps )) +
  geom_histogram() +
  labs(title = "Mean with NAs", x = "Mean per day", y = "")

p4 <- ggplot(median_filled_NA, aes(x = steps )) +
  geom_histogram() +
  labs(title = "Median with NAs", x = "Median per day", y = "")

p1+p2+p3+p4

```

Finally, the last part: Are there differences in activity patterns between weekdays and weekends?
...
Second, make a panel plot containing a time series plot (i.e. type = "l") of the 5-minute interval (x-axis) and the average number of steps taken, averaged across all weekday days or weekend days (y-axis).
